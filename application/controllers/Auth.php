...
//token
	 		...
	 		$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">Selamat, silahkan aktivasi akun anda! </div>');
 			redirect('auth');
 	...
 	private function _sendEmail($token, $type)
 	{
        ...
        $config['smtp_user'] = 'remedialpakarif@gmail.com';
        $config['smtp_pass'] = 'Remedialpakarif123';
        $config['smtp_port'] = 465;
        $config['mailtype'] = 'html';
        $config['charset'] = 'utf-8';
        $this->email->initialize($config);
        $this->email->set_newline("\r\n"); 	
		$this->email->from('remedialpakarif@gmail.com', 'Remedial pakArif');
	...
	if ($user) { 
		...
		if($user_token) {
			...
			if(time() - $user_token['date_created'] < (60*60*24)) {
				...
				$this->session->set_flashdata('message','<div class="alert alert-success" role="alert">'. $email .'Telah diaktivkan! Silahkan login. </div>');
		redirect('auth');
